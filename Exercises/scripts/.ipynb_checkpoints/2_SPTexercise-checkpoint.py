...
"""
Import wavelet filtered image
"""

#imgWL = tiff.imread('data/TheOthersFromFirstRun/wt9_300stack.tif')
imgWL = tiff.imread('data/TheGoodOne/wt_100stack.tif')
nFrames = 100

# eIF4E - 08-14 imaging (adjust these parameters based on image acquisition)
ExpInfo.scale = 0.094
ExpInfo.unit = '\mu m'
...
"""
df = pd.read_csv("data/TheGoodOne/wt_100stack_Tracks2.csv")
...
def plot_trajectories_roi(
        trajs,
        background_image=None,
        roi_center=None,          # (x,y) center
        roi_width=None,           # pixel width
    ):  
    """
    Plot all trajectories. Optionally, zoom into a Region of Interest (ROI).

    trajs: list of trajectories, each a list of (x,y) coordinates.
    background_image: 2D/3D numpy array (optional).
    roi_center: (x,y) center of ROI (optional).
    roi_width: width of ROI box in pixels (optional).
    """
    ...
    for traj in trajs:
        ...
        plt.plot(xs, ys, linewidth=1)
        plt.scatter(xs[0], ys[0], marker='o', s=40)
        plt.scatter(xs[-1], ys[-1], marker='x', s=50)

    if roi_center is not None and roi_width is not None:
        cx, cy = roi_center
        half = roi_width / 2
        ...
        roi_rect = Rectangle(
            (x_min, y_min), roi_width, roi_width,
        ...
        plt.ylim(y_max, y_min)  # inverted y-axis for image coordinate system

    else:
        # Default: invert y-axis just once
...
pointToView = 16
roi = pCMs[0][pointToView,:]  
plot_trajectories_roi(trajs, background_image=imgWL[0], roi_center=roi, roi_width=roiWidth)
...
# Only take trajectories with greater than 5 points 
